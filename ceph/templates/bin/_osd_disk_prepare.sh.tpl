...
function osd_disk_prepare {
  ...
  if [ -b "${OSD_JOURNAL}" -a "${OSD_FORCE_ZAP:-0}" -eq 1 ]; then
    ...
    # because we are likely working on a partition, though, we'll just
    # make a filesystem on the device, as other partitions may be
    # in usse so zapping the whole dev is unsafe.
